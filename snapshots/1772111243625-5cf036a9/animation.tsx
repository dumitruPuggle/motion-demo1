...
export const AgentCard: React.FC<{
	title?: string;
	subtitle?: string;
	accent?: "purple" | "gold" | "pink" | "blue";
}> = ({
	title = "Agent Swarm",
	subtitle = "Simple RAG Agent",
	accent = "purple",
}) => {
	const {width} = useVideoConfig();
	const cardW = Math.min(720, Math.max(520, width - 200));

	const accentGradient =
		accent === "gold"
			? "linear-gradient(90deg, #d7b060 0%, #ffd89b 45%, #b57d59 100%)"
			: accent === "pink"
				? "linear-gradient(90deg, #c07ab6 0%, #ffb6f2 45%, #7a4dff 100%)"
				: accent === "blue"
					? "linear-gradient(90deg, #4e79ff 0%, #8bb7ff 50%, #7a4dff 100%)"
					: "linear-gradient(90deg, #7a4dff 0%, #c07ab6 45%, #d7b060 100%)";
		...
		>
			<div
				style={{
					...
					background: `linear-gradient(rgba(18,22,52,0.62), rgba(18,22,52,0.62)) padding-box, ${accentGradient} border-box`,
					pointerEvents: "none",
					opacity: 0.95,
				}}
			/>
			...
			>
				<div
					style={{
						...
						boxShadow:
							"0 0 0 2px rgba(255,255,255,0.06) inset, 0 10px 24px rgba(0,0,0,0.22)",
						display: "flex",
						alignItems: "center",
...
export const SimpleRagAgentCard: React.FC = () => {
	return (
		<AgentCard title="Simple RAG" subtitle="Retrieve → Ground → Answer" accent="purple" />
	);
};
