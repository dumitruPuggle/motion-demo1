...
// ---------------------------------------------------------------------------
// Referenced component: SimpleRagAgentCard (AgentCard)
// Fix: remove unused AbsoluteFill import (was causing TS/ESLint compile failure)
// ---------------------------------------------------------------------------
...
const IconSvg: React.FC<React.PropsWithChildren<{size: number; viewBox?: string}>> =
	...
	return (
		<IconSvg size={size}>
			...
			<path
				...
				opacity={0.82}
			/>
			<path d="M4 21.2h18v1.2H4v-1.2Z" fill={color} opacity={0.95} />
...
export const AssistantLanding: React.FC = () => {
	...
	const transitionStart = Math.max(0, durationInFrames - Math.round(fps * 0.9));
	const transitionT = clamp(
		(frame - transitionStart) / (durationInFrames - transitionStart),
		0,
		1
	);
	const transitionEase = easeOutCubic(transitionT);

	// Scene A (landing) transitions out
	const globalOpacity = interpolate(transitionEase, [0, 1], [1, 0]);
	const globalY = interpolate(transitionEase, [0, 1], [0, 18]);

	// Scene B (agent card) transitions in
	const nextOpacity = interpolate(transitionEase, [0, 1], [0, 1]);
	const nextY = interpolate(transitionEase, [0, 1], [18, 0]);
	const nextScale = interpolate(transitionEase, [0, 1], [0.985, 1]);
	...
	return (
		...
		>
			...
			>
				<AnimatedIn frame={frame} fps={fps} delay={6} from={{y: 26, scale: 0.985, blur: 14}}>
					<div
						style={{
							width: cardW,
							height: cardH,
					...
					>
						...
						<div style={{position: "absolute", left: 34, top: 134}}>
							<AnimatedIn frame={frame} fps={fps} delay={20} from={{y: 16, scale: 0.98, blur: 10}}>
								<div
									style={{
										height: 45,
										borderRadius: 999,
								...
								>
									<AnimatedIn frame={frame} fps={fps} delay={26} from={{x: -10, y: 0, scale: 0.9, blur: 8}}>
									...
									>
										<AnimatedIn frame={frame} fps={fps} delay={28} from={{y: 8, blur: 10}}>
											<span style={{display: "inline-block"}}>Agent Swarm</span>
										</AnimatedIn>
									</div>
								</div>
							</AnimatedIn>
						...
						>
							<AnimatedIn frame={frame} fps={fps} delay={30} from={{x: 18, y: 0, scale: 0.96, blur: 10}}>
							...
							<AnimatedIn frame={frame} fps={fps} delay={34} from={{x: 18, y: 0, scale: 0.96, blur: 10}}>
							...
							<AnimatedIn frame={frame} fps={fps} delay={38} from={{x: 20, y: 0, scale: 0.97, blur: 10}}>
								<div
									style={{
										height: 54,
										width: 196,
										...
										gap: 10,
										backdropFilter: "blur(6px)",
										boxShadow: `0 ${10 + borderPulse * 6}px ${24 + borderPulse * 10}px rgba(0,0,0,0.22)`,
									}}
								>
									<div
										style={{
											...
											fontWeight: 700,
											letterSpacing: -0.2,
											transform: `translateY(${interpolate(borderPulse, [0, 1], [0, -0.5])}px)`,
										}}
									>
										Send
									</div>
			...
			{/* Scene B: crossfade/slide in near the end */}
			<Sequence from={transitionStart}>
				<div
					style={{
						opacity: nextOpacity,
						transform: `translate3d(0, ${nextY}px, 0) scale(${nextScale})`,
						willChange: "transform, opacity",
					}}
				>
					<SimpleRagAgentCard />
